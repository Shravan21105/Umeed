...
function Navbar({ darkMode, setDarkMode }) {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 w-full z-50 p-5 shadow-lg transition-all duration-300 ${
      ...
      >
        <div className="container mx-auto flex justify-between items-center relative">
          ...
          >
            <span
              className="bg-gradient-to-r from-blue-700 to-pink-500 
          ...
          <div
            className={`absolute top-[100%] left-1/2 transform -translate-x-1/2 bg-gray-100 shadow-lg rounded-lg py-3 w-40 transition-all duration-300 ${
              isOpen ? "block" : "hidden"
            } lg:hidden`}
          >
              ...
              {menuItems.map((item, index) => {
                ...
                return (
                  <li key={index} className="relative w-full text-center">
                    <Link
                      to={path}
                      className={`block w-full py-2 font-medium bg-clip-text text-black ${
                        darkMode
